    ...
    <thead>
        <tr>
            ...
            <?php foreach ($problems as $key => $p) : ?>
                <th>
                    <?= Html::a('P' . ($key + 1), ['/training/problem', 'id' => $model->id, 'pid' => $key],['data-pjax' => '0']) ?>
                    <br>
        ...
        这时不告知用户排名，只打印 ? 号。 -->
            ...
            <tr <?php if (!$autoRefresh) : ?> class="animate__animated animate__fadeInUp"<?php endif; ?> >
                ...
                foreach ($problems as $key => $p) {
                    ...
                    if ($num != '' && ((!Yii::$app->user->isGuest && $model->created_by == Yii::$app->user->id) || (!$model->isScoreboardFrozen() && $model->isContestEnd()))) {
                        $url = Url::toRoute([
                            '/contest/submission',
                            'pid' => $p['problem_id'],
                            'cid' => $model->id,
                        ...
                        echo "<th class=\"table-problem-cell {$css_class}\" style=\"cursor:pointer\" onclick='submission_click(this)' data-href='{$url}'>{$num} {$span}</th>";
                    } else {
                        echo "<th class=\"table-problem-cell {$css_class}\">{$num} {$span}</th>";
                    }
                }
                ?>
            </tr>
