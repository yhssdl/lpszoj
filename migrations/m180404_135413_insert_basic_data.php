...
class m180404_135413_insert_basic_data extends BaseMigration
{
    public function up()
    {
        ...
        $this->insert('{{%problem}}', [
            ...
            'spj' => 0,
            'hint' => "<p>通常，题目会要求多组样例输入。对于多组样例输入，一般会是读到 <code>EOF</code> 结束。 <code>EOF</code> 的意思是 <code>End Of File</code>，表示读到文件尾，结束输入。 <code>scanf</code> 函数的返回值如果为 <code>EOF</code> 的话，就表示输入结束了。比如题目输入一个数，以 <code>EOF</code> 结束，你就可以这样写：</p><p>C 语言：</p><pre><code class=\"language-c\"><p>#include&lt;stdio.h&gt;<br>int main()<br>{<br>    int n;<br>    while (scanf(\"%d\", &amp;n) != EOF) {<br>         //解题代码<br>    }<br>    return 0;<br>}</p></code></pre><p>C++:&nbsp;</p><pre><code class=\"language-cpp\"><p>#include &lt;iostream&gt;<br>using namespace std;<br>int main()<br>{<br>    int n;<br>    while (cin &gt;&gt; n) {<br>        //解题代码<br>    }<br>    return 0;<br>}</p></code></pre><p>所以，这道题的 Accepted 代码是（以Ｃ语言为例）：</p><pre><code class=\"language-c\"><p>#include&lt;stdio.h&gt;<br>int main()<br>{<br>    int n;<br>    while (scanf(\"%d\", &amp;n) != EOF) {<br>        printf(\"%d\\n\", n * (n + 1) / 2 );<br>    }<br>    return 0;<br>}</p></code></pre>",
            'time_limit' => 1,
            'memory_limit' => 128,
            'status' => 1,
            'created_at' => $time,
        ...
        $this->insert('{{%problem}}', [
            ...
            'spj' => 0,
            'hint' =>  "<p>题目要求输入 \$t\$ 组样例，那么我们可以这样写：</p><pre><code class=\"language-c\"><p>#include &lt;stdio.h&gt;<br>int main()<br>{<br>    int t, n;<br>    scanf(\"%d\", &amp;t);<br>    while (t--) {<br>        scanf(\"%d\", &amp;n);<br>        //在这里写判断 n 是否为闰年的代码及输出结果<br>    }<br>    return 0;<br>}</p></code></pre><p>这道题的解题方法就靠大家发挥了．</p><p>提示：在处理多组样例时，可以一组样例、一组样例地输出，而不必等处理完所有样例才统一输出。</p>",
            'time_limit' => 1,
            'memory_limit' => 128,
            'status' => 1,
            'created_at' => $time,
